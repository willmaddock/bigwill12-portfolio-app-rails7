...
RSpec.describe "Students", type: :request do
  ...
  describe "GET /students (search functionality)" do
    ...
    let!(:student3) { Student.create!(first_name: "Michael", last_name: "Jordan", school_email: "jordan@msudenver.edu", major: "Computer Engineering BS", graduation_date: "2024-05-15") }

    before do
      # Optionally set a per_page value for pagination tests
      @per_page = 10
    end

    # Test 5: Search by major
    it "returns students matching the major" do
      get students_path, params: { search: { major: "Computer Science BS" }, page: 1, per_page: @per_page }
    ...
    it "returns students graduating before the given date" do
      get students_path, params: { search: { expected_graduation_date: "2025-01-01", date_type: "before" }, page: 1, per_page: @per_page }
      expect(response.body).to include("Aaron") # Michael graduates before 2025-01-01
      expect(response.body).to include("Jackie") # Ensure Aaron is not included
    end

    # Test 7: Search by expected graduation date (after)
    it "returns students graduating after the given date" do
      get students_path, params: { search: { expected_graduation_date: "2024-01-01", date_type: "after" }, page: 1, per_page: @per_page }
      expect(response.body).to include("Michael") # Aaron graduates after 2024-01-01
      expect(response.body).to include("Aaron") # Ensure Michael is not included
    end
  end
